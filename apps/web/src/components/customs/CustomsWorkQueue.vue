...
defineProps<{
  items: readonly CustomsQueueItem[];
  selectedTaskId: string;
  loading: boolean;
}>();
const emit = defineEmits<{ select: [item: CustomsQueueItem] }>();
</script>

<template>
  <section class="queue" aria-label="清关任务列表">
    <header>
      <span>清关任务池</span><b>{{ items.length }}</b>
    </header>
    <p v-if="loading" class="empty">正在加载清关任务…</p>
    <p v-else-if="!items.length" class="empty">当前没有待处理的清关任务。</p>
    <button
      v-for="item in items"
      v-else
      :key="item.task.id"
