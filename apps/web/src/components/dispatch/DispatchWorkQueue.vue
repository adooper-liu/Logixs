...
const emit = defineEmits<{ select: [item: DispatchQueueItem] }>();
</script>

<template>
  <section class="queue" aria-label="出运任务列表">
    <header>
      <span>出运任务池</span><b>{{ items.length }}</b>
    </header>
    <p v-if="loading" class="empty">正在加载出运任务…</p>
    <p v-else-if="!items.length" class="empty">当前没有待处理的出运任务。</p>
    <button
      v-for="item in items"
      v-else
      :key="item.task.id"
