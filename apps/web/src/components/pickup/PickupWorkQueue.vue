...
import { Truck, TriangleAlert } from "@lucide/vue";
import type { PickupQueueItem } from "../../data/pickupWorkbench";

defineProps<{
  items: readonly PickupQueueItem[];
  selectedTaskId: string;
  loading: boolean;
}>();
const emit = defineEmits<{ select: [item: PickupQueueItem] }>();
</script>

<template>
  <section class="queue" aria-label="提柜任务列表">
    <header>
      <span>提柜任务池</span><b>{{ items.length }}</b>
    </header>
    <p v-if="loading" class="empty">正在加载提柜任务…</p>
    <p v-else-if="!items.length" class="empty">当前没有待处理的提柜任务。</p>
    <button
      v-for="item in items"
      v-else
      :key="item.task.id"
