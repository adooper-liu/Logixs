...
export function useStuffingTaskOperation(
  ...
  async function execute(reuseKey = false): Promise<void> {
    ...
    const idempotencyKey = nextKey(action.workOrderId, reuseKey);
    try {
      if (action.actionCode === CLAIM_WORK_ORDER_ACTION) {
        remember(
          ...
          toClaimSubmission({
            ...
            result: await claimWorkOrder(action.workOrderId, {
              idempotencyKey,
            }),
        ...
        remember(
          ...
          toCompleteSubmission({
            taskId: currentTask.id,
            result: await completeWorkOrder(action.workOrderId, {
              evidenceRefs: [...(snapshot.value?.evidenceRefs ?? [])],
              idempotencyKey,
            }),
            observedAt: observedAt(),
          }),
  ...
  function nextKey(workOrderId: string, reuse: boolean): string {
  ...
  return { submission, submitting, execute, retry };
}
