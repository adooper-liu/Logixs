...
服务起来后，一个入口搞定开发调试（不记一堆 URL）：

| 入口          | 地址                             | 说明                                |
| ------------- | -------------------------------- | ----------------------------------- |
| 开发控制台    | `http://localhost:5173/dev`      | 服务状态 + 工具入口 + 端口/命令速查 |
| Swagger       | `http://localhost:3000/api/docs` | API 文档与在线调试                  |
| Prisma Studio | `pnpm db:studio` → `:5555`       | 数据库可视化                        |
| Temporal UI   | `http://localhost:8233`          | 工作流可视化                        |
