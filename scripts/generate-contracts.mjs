import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
import { relative, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { compile } from "json-schema-to-typescript";
import { format } from "prettier";

// 单一权威源：14 个 JSON Schema 2020-12 文件。本脚本把它们打包成一个自洽根 Schema，
// 再由 json-schema-to-typescript 生成一份去重后的 TypeScript 类型（共享类型只定义一次）。
// 生成文件是派生产物，禁止手工修改；漂移由 `--check` 模式在 CI 阻断。

const root = fileURLToPath(new URL("..", import.meta.url));
const schemaRoot = resolve(root, "packages/contracts/schemas/v1");
const outDir = resolve(root, "packages/contracts/generated");
const outFile = resolve(outDir, "contracts.d.ts");
const importFieldCatalogPath = resolve(
  root,
  "packages/contracts/catalogs/v1/import-fields.json",
);
const typescriptImportFieldCatalogPath = resolve(
  root,
  "packages/contracts/import-fields-json.d.ts",
);
const canonicalEventCatalogPath = resolve(
  root,
  "packages/contracts/catalogs/v1/canonical-events.json",
);
const typescriptCanonicalEventCatalogPath = resolve(
  root,
  "packages/contracts/canonical-events-json.d.ts",
);
const lifecycleNodeCatalogPath = resolve(
  root,
  "packages/contracts/catalogs/v1/lifecycle-nodes.json",
);
const typescriptLifecycleNodeCatalogPath = resolve(
  root,
  "packages/contracts/lifecycle-nodes-json.d.ts",
);
const pythonImportFieldCatalogPath = resolve(
  root,
  "apps/ai-service/app/generated_import_field_catalog.py",
);

// 参与类型生成的是公共契约 Schema；index.json 只是治理索引，
// scenario-fixture / schema-fixture-suite 是测试基础设施，均不生成业务类型。
const CONTRACT_FILES = [
  "common.schema.json",
  "lifecycle-state-machine.schema.json",
  "canonical-event-envelope.schema.json",
  "lifecycle-timeline.schema.json",
  "work-execution.schema.json",
  "evidence-record.schema.json",
  "cross-module-reference.schema.json",
  "action-command.schema.json",
  "client-operation.schema.json",
  "container-operational-view.schema.json",
  "ops-assistant.schema.json",
  "error-response.schema.json",
];

const pascalName = (filename) =>
  filename
    .replace(/\.schema\.json$/, "")
    .split("-")
    .map((part) => part.charAt(0).toUpperCase() + part.slice(1))
    .join("");

const readJson = (path) => JSON.parse(readFileSync(path, "utf8"));

const loadSchemas = () => {
  const schemas = new Map();
  for (const filename of CONTRACT_FILES) {
    schemas.set(filename, readJson(resolve(schemaRoot, filename)));
  }
  return schemas;
};

// 顶层是 `type: object` 的文件需要为整份 DTO 生成一个命名类型；其余文件只是 $defs 容器。
const topLevelTypeName = (schema, filename) =>
  schema.type === "object" ? pascalName(filename) : undefined;

const buildTopLevelNames = (schemas) => {
  const names = new Map();
  for (const [filename, schema] of schemas) {
    const name = topLevelTypeName(schema, filename);
    if (name) names.set(filename, name);
  }
  return names;
};

const pascalProperty = (name) =>
  name
    .split(/[-_]/)
    .map((part) => part.charAt(0).toUpperCase() + part.slice(1))
    .join("");

// 收集被 `#/properties/X` 引用的属性目标，统一为其注入 title，
// 避免 jstt 对内联对象自动命名为 Source/Source1 之类无意义名字。
const collectPropertyRefTargets = (schemas) => {
  const targets = new Map(); // file -> Set(propName)
  const record = (file, prop) => {
    if (!targets.has(file)) targets.set(file, new Set());
    targets.get(file).add(prop);
  };
  const walk = (value, currentFile) => {
    if (!value || typeof value !== "object") return;
    if (typeof value.$ref === "string") {
      const hashAt = value.$ref.indexOf("#");
      const targetFile = hashAt < 0 ? value.$ref : value.$ref.slice(0, hashAt);
      const fragment = hashAt < 0 ? "" : value.$ref.slice(hashAt);
      const match = fragment.match(/^#\/properties\/([^/]+)$/);
      if (match) record(targetFile || currentFile, match[1]);
    }
    for (const child of Object.values(value)) walk(child, currentFile);
  };
  for (const [filename, schema] of schemas) walk(schema, filename);
  return targets;
};

const injectPropertyTitles = (schemas, topLevelNames) => {
  const targets = collectPropertyRefTargets(schemas);
  for (const [filename, propNames] of targets) {
    const schema = schemas.get(filename);
    for (const prop of propNames) {
      const propSchema = schema?.properties?.[prop];
      if (propSchema && typeof propSchema === "object") {
        propSchema.title = `${topLevelNames.get(filename)}${pascalProperty(prop)}`;
      }
    }
  }
};

const stripMeta = (value) => {
  if (!value || typeof value !== "object") return value;
  if (Array.isArray(value)) return value.map(stripMeta);
  const out = {};
  for (const [key, child] of Object.entries(value)) {
    if (key === "$schema" || key === "$id" || key.startsWith("x-")) continue;
    out[key] = stripMeta(child);
  }
  return out;
};

const rewriteRef = (ref, currentFilename, topLevelNames) => {
  if (typeof ref !== "string") return ref;
  const hashAt = ref.indexOf("#");
  const targetFile = hashAt < 0 ? ref : ref.slice(0, hashAt);
  const fragment = hashAt < 0 ? "" : ref.slice(hashAt);

  const syntheticNameFor = (filename) =>
    topLevelNames.get(filename) ?? pascalName(filename);

  if (!targetFile) {
    // 同文件本地引用
    if (!fragment || fragment === "#") return ref;
    if (fragment.startsWith("#/$defs/")) return ref; // 扁平合并后仍指向根 $defs
    // 本地属性级引用（如 #/properties/source）：改指本文件的顶层合成类型
    return `#/$defs/${syntheticNameFor(currentFilename)}${fragment.slice(1)}`;
  }

  // 跨文件引用
  if (!fragment || fragment === "#") {
    return `#/$defs/${syntheticNameFor(targetFile)}`;
  }
  if (fragment.startsWith("#/$defs/")) {
    return `#/$defs/${fragment.slice("#/$defs/".length)}`;
  }
  return `#/$defs/${syntheticNameFor(targetFile)}${fragment.slice(1)}`;
};

const deepRewrite = (value, currentFilename, topLevelNames) => {
  if (!value || typeof value !== "object") return value;
  if (Array.isArray(value)) {
    return value.map((item) =>
      deepRewrite(item, currentFilename, topLevelNames),
    );
  }
  const out = {};
  for (const [key, child] of Object.entries(value)) {
    out[key] =
      key === "$ref"
        ? rewriteRef(child, currentFilename, topLevelNames)
        : deepRewrite(child, currentFilename, topLevelNames);
  }
  return out;
};

const buildBundle = (schemas, topLevelNames) => {
  const defs = {};
  for (const [filename, schema] of schemas) {
    for (const [defName, defValue] of Object.entries(schema.$defs ?? {})) {
      if (defName in defs) throw new Error(`duplicate $defs name: ${defName}`);
      defs[defName] = deepRewrite(stripMeta(defValue), filename, topLevelNames);
    }
  }
  for (const [filename, name] of topLevelNames) {
    const schema = schemas.get(filename);
    const topLevel = deepRewrite(stripMeta(schema), filename, topLevelNames);
    delete topLevel.$defs;
    delete topLevel.title; // 用 $defs 键名统一命名，避免 title 的 V1 后缀产生不一致
    defs[name] = topLevel;
  }
  return { title: "LogixContractsV1", type: "object", $defs: defs };
};

const banner = `/* eslint-disable */\n/* prettier-ignore */\n// 由 scripts/generate-contracts.mjs 生成，禁止手工修改。\n// 权威源：packages/contracts/schemas/v1/*.schema.json\n`;

const generateTs = async () => {
  const schemas = loadSchemas();
  const topLevelNames = buildTopLevelNames(schemas);
  injectPropertyTitles(schemas, topLevelNames);
  const bundle = buildBundle(schemas, topLevelNames);
  const ts = await compile(bundle, "LogixContractsV1", {
    bannerComment: "",
    unreachableDefinitions: true,
    format: false,
  });
  return `${banner}${ts.replace(/^\/\*\*[\s\S]*?\*\/\n\n/gm, "")}`;
};

const generatePythonImportFieldCatalog = () => {
  const catalog = readJson(importFieldCatalogPath);
  const rules = catalog.fields.map(
    ({ code, aliases }) =>
      `    (${JSON.stringify(code)}, (${aliases.map((alias) => JSON.stringify(alias)).join(", ")},)),`,
  );
  return [
    "# Generated by scripts/generate-contracts.mjs. Do not edit.",
    "# Authority: packages/contracts/catalogs/v1/import-fields.json",
    `IMPORT_FIELD_CATALOG_VERSION = ${JSON.stringify(catalog.version)}`,
    "IMPORT_FIELD_ALIAS_RULES: tuple[tuple[str, tuple[str, ...]], ...] = (",
    ...rules,
    ")",
    "",
  ].join("\n");
};

const generateTypescriptImportFieldCatalog = () => {
  const catalog = readJson(importFieldCatalogPath);
  const union = (values) =>
    values.map((value) => JSON.stringify(value)).join(" | ");
  const multilineUnion = (values) =>
    values.map((value) => `  | ${JSON.stringify(value)}`).join("\n");
  return [
    "// Generated by scripts/generate-contracts.mjs. Do not edit.",
    "// Authority: packages/contracts/catalogs/v1/import-fields.json",
    `export type ImportFieldCode =\n${multilineUnion(catalog.fields.map(({ code }) => code))};`,
    `export type ImportFieldScope = ${union([...new Set(catalog.fields.map(({ scope }) => scope))])};`,
    `export type QuantityUnitCode = ${union(catalog.quantityUnits.map(({ code }) => code))};`,
    `export type ImportTimeFactCode =\n${multilineUnion(catalog.timeFacts.map(({ code }) => code))};`,
    `export type ImportTimeKind = ${union([...new Set(catalog.timeFacts.map(({ timeKind }) => timeKind))])};`,
    `export type ImportTimeCaptureSource = ${union([...new Set(catalog.timeFacts.map(({ captureSource }) => captureSource))])};`,
    "export interface ImportFieldDefinition {",
    "  code: ImportFieldCode;",
    "  label: string;",
    "  scope: ImportFieldScope;",
    "  required: boolean;",
    "  aliases: string[];",
    "}",
    "export interface QuantityUnitDefinition {",
    "  code: QuantityUnitCode;",
    "  label: string;",
    "  aliases: string[];",
    "}",
    "export interface ImportTimeFactDefinition {",
    "  code: ImportTimeFactCode;",
    "  label: string;",
    "  timeFieldCode: ImportFieldCode;",
    "  statusFieldCode?: ImportFieldCode;",
    "  completedStatusAliases: string[];",
    "  timeKind: ImportTimeKind;",
    "  captureSource: ImportTimeCaptureSource;",
    "  eventCode: string | null;",
    "  requiresEvidence: boolean;",
    "  requiresDerivationRule: boolean;",
    "}",
    "export interface ImportFieldCatalog {",
    "  version: string;",
    "  fields: ImportFieldDefinition[];",
    "  quantityUnits: QuantityUnitDefinition[];",
    "  timeFacts: ImportTimeFactDefinition[];",
    "}",
    "declare const catalog: ImportFieldCatalog;",
    "export default catalog;",
    "",
  ].join("\n");
};

const generateTypescriptCanonicalEventCatalog = async () => {
  const catalog = readJson(canonicalEventCatalogPath);
  const typeAlias = (name, values) =>
    [
      `export type ${name} =`,
      ...[...new Set(values)].map((value) => `  | ${JSON.stringify(value)}`),
    ].join("\n") + ";";
  const source = [
    "// Generated by scripts/generate-contracts.mjs. Do not edit.",
    "// Authority: packages/contracts/catalogs/v1/canonical-events.json",
    typeAlias(
      "CatalogEventCode",
      catalog.map(({ eventCode }) => eventCode),
    ),
    typeAlias(
      "CatalogEventOwnerDomain",
      catalog.map(({ ownerDomain }) => ownerDomain),
    ),
    typeAlias(
      "CatalogEventRole",
      catalog.map(({ role }) => role),
    ),
    typeAlias(
      "CatalogTimeKind",
      catalog.flatMap(({ allowedTimeKinds }) => allowedTimeKinds),
    ),
    "export interface CanonicalEventCatalogItem {",
    "  eventCode: CatalogEventCode;",
    "  eventVersion: 1;",
    "  nameCn: string;",
    "  definition: string;",
    "  role: CatalogEventRole;",
    "  ownerDomain: CatalogEventOwnerDomain;",
    "  defaultNodeCode: string | null;",
    "  allowedTimeKinds: CatalogTimeKind[];",
    "  completionEligibleNodeCodes: string[];",
    "  provenance: string[];",
    "}",
    "declare const catalog: CanonicalEventCatalogItem[];",
    "export default catalog;",
    "",
  ].join("\n");
  return format(source, { parser: "typescript" });
};

const generateTypescriptLifecycleNodeCatalog = async () => {
  const catalog = readJson(lifecycleNodeCatalogPath);
  const applicability = [...new Set(catalog.map((node) => node.applicability))]
    .map((value) => JSON.stringify(value))
    .join(" | ");
  const source = [
    "// Generated by scripts/generate-contracts.mjs. Do not edit.",
    "// Authority: packages/contracts/catalogs/v1/lifecycle-nodes.json",
    'import type { CompletionMode, LifecycleNodeCode } from ".";',
    `export type CatalogNodeApplicability = ${applicability};`,
    "export interface LifecycleNodeCatalogItem {",
    "  sequence: number;",
    "  nodeCode: LifecycleNodeCode;",
    "  applicability: CatalogNodeApplicability;",
    "  completionMode: CompletionMode;",
    "}",
    "declare const catalog: LifecycleNodeCatalogItem[];",
    "export default catalog;",
    "",
  ].join("\n");
  return format(source, { parser: "typescript" });
};

const main = async () => {
  const checkOnly = process.argv.includes("--check");
  const generated = await generateTs();
  const generatedPythonCatalog = generatePythonImportFieldCatalog();
  const generatedTypescriptCatalog = generateTypescriptImportFieldCatalog();
  const generatedCanonicalEventCatalog =
    await generateTypescriptCanonicalEventCatalog();
  const generatedLifecycleNodeCatalog =
    await generateTypescriptLifecycleNodeCatalog();

  if (checkOnly) {
    if (!existsSync(outFile)) {
      console.error(
        "contracts.d.ts missing; run `pnpm contract:generate` first",
      );
      process.exitCode = 1;
      return;
    }
    const committed = readFileSync(outFile, "utf8");
    if (committed !== generated) {
      console.error(
        "contracts.d.ts drift detected; run `pnpm contract:generate` and commit the result",
      );
      process.exitCode = 1;
    } else {
      console.log("Contract types in sync with JSON Schema authority.");
    }
    if (!existsSync(pythonImportFieldCatalogPath)) {
      console.error(
        "generated_import_field_catalog.py missing; run `pnpm contract:generate` first",
      );
      process.exitCode = 1;
    } else if (
      readFileSync(pythonImportFieldCatalogPath, "utf8") !==
      generatedPythonCatalog
    ) {
      console.error(
        "Python import field catalog drift detected; run `pnpm contract:generate` and commit the result",
      );
      process.exitCode = 1;
    }
    if (!existsSync(typescriptImportFieldCatalogPath)) {
      console.error(
        "import-fields-json.d.ts missing; run `pnpm contract:generate` first",
      );
      process.exitCode = 1;
    } else if (
      readFileSync(typescriptImportFieldCatalogPath, "utf8") !==
      generatedTypescriptCatalog
    ) {
      console.error(
        "TypeScript import field catalog drift detected; run `pnpm contract:generate` and commit the result",
      );
      process.exitCode = 1;
    }
    if (!existsSync(typescriptCanonicalEventCatalogPath)) {
      console.error(
        "canonical-events-json.d.ts missing; run `pnpm contract:generate` first",
      );
      process.exitCode = 1;
    } else if (
      readFileSync(typescriptCanonicalEventCatalogPath, "utf8") !==
      generatedCanonicalEventCatalog
    ) {
      console.error(
        "TypeScript canonical event catalog drift detected; run `pnpm contract:generate` and commit the result",
      );
      process.exitCode = 1;
    }
    if (!existsSync(typescriptLifecycleNodeCatalogPath)) {
      console.error(
        "lifecycle-nodes-json.d.ts missing; run `pnpm contract:generate` first",
      );
      process.exitCode = 1;
    } else if (
      readFileSync(typescriptLifecycleNodeCatalogPath, "utf8") !==
      generatedLifecycleNodeCatalog
    ) {
      console.error(
        "TypeScript lifecycle node catalog drift detected; run `pnpm contract:generate` and commit the result",
      );
      process.exitCode = 1;
    }
    return;
  }

  mkdirSync(outDir, { recursive: true });
  writeFileSync(outFile, generated);
  writeFileSync(pythonImportFieldCatalogPath, generatedPythonCatalog);
  writeFileSync(typescriptImportFieldCatalogPath, generatedTypescriptCatalog);
  writeFileSync(
    typescriptCanonicalEventCatalogPath,
    generatedCanonicalEventCatalog,
  );
  writeFileSync(
    typescriptLifecycleNodeCatalogPath,
    generatedLifecycleNodeCatalog,
  );
  console.log(
    `Generated ${relative(root, outFile)} (${generated.split("\n").length} lines).`,
  );
};

main().catch((error) => {
  console.error(error.stack ?? error.message);
  process.exitCode = 1;
});
