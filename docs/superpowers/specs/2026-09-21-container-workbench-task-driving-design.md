...
纪律：轨道必须区分**已完 / 当前 / 未发生**，**未发生的节点不得画成已完成**（`UI_SYSTEM §5`）。"计划 / 预计 / 实际"三轨的可行性见 §7.2——**一期只能画实际日期**，预计轨需新投影，计划轨无数据来源。
...
2. **轨道三轨（计划 / 预计 / 实际）里，"计划轨"没有数据来源。** 四层全断，且三种时间的供给完全不同：

   **结构上画不出来**：`NodeInstance` 表只有 `state` / `applicability` / `completedAt`，**没有时间列**（时间属于事实流水账，不属于节点实例，这是有意的设计）。`LifecycleNodeItem` 与 `LiveNodeView` 同样除 `completedAt` 外无时间字段，`LiveNodeRail.vue` 模板里也没有渲染时间的标记。要画三轨需新增一个按节点聚合时间事实的投影，并打通 DTO → 视图模型 → 组件三层。
   ...
   **数据供给上**：
   ...
   "计划时间"是**模型预留但从未被写入**的空槽位——系统里没有"计划编制"这个业务动作。画计划轨等于先发明一个新业务动作，**超出本设计范围**。

   **结论**：一期只能画"已完 / 当前 / 未发生 + 已落库实际日期"；二期加投影后可补"预计轨"（覆盖不全）；**"计划轨"需先有计划编制，不在本设计内**。
