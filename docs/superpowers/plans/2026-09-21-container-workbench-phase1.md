...
- [x] 运行态确认：货柜 `458df02f-3b59-435f-950a-3d7321795310` 的 `container_unloading` 节点、匹配 NodeTask 和工单均为 `completed`，且无下一动作；前端 `TaskQueue` 回归确认 completed 不进入待处理区
- [x] 运行态确认：新柜 `10000000-0000-4000-8000-000000000001` 返回完整 **14 站**和 14 组全空三轨；前端投影、轨道、三轨卡与页面测试确认空值写作 `—`。中转/海铁默认保持 `optional_applicable`；只有证据与 `SetNodeApplicability` 确认后才显示“不适用”，不得把新建柜的未知路线伪造成 N/A

验收边界：当前会话无可连接的交互式浏览器，未新增实机目视证据；此前已通过的页面级 Playwright 桌面、窄屏和移动端验收保持有效。本次以真实 API 样本加 5 个前端定向测试文件 26 项通过完成运行态与投影的组合核对，未修改数据库制造 N/A 样本。
