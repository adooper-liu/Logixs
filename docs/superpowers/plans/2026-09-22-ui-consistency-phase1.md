...
**实测规模（检查器实现后扫全量 106 个文件）：1380 处违规，92 个文件；字号 360 / 间距 1020；88% 在 `components/`。**

排期按**组件规模**而非页面（违规按组件聚集，不是按页面均布）：

| 阶段 | 目录 | 违规量 |
| ---- | ---- | ------ |
| 2 | `components/task` + `components/container` | 351 |
| 3 | `components/cargo-ready` + `stuffing` + `management` | 364 |
| 4 | 其余 `components/*` + `views/` + `themes/` + `styles/` | 609 |
| 5 | 删基线机制与冻结快照，门禁转硬限制 | — |

每迁完一个目录就从基线删掉对应条目。详见 spec `§6 分期（棘轮）`。
