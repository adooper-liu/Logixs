...
- **计费口径（2026-09-05 建议，待定稿）**：`费用 = max(0, 结算截止 − 起算 − 免费天数) × 日费率`；起算基准＝`按到港/按卸船`；免费基准＝`自然日/工作日`(现网仅自然日)；**预计**(计划时点×标准,事前干预)→**实际**(实际时点×账单,对账复盘)同引擎；聚合＝按月/按柜/按柜·天；清单见 [FEE_DEMURRAGE](./domain/FEE_DEMURRAGE.md)。
