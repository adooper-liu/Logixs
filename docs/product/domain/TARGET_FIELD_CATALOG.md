...
> 关键输入（负责人确认）：**备货单身份/旧导入分组锚 = 备货单号**（采购阶段=采购订单号 PO），货柜稳定身份为 `containerId`；同一备货单可因多个产品货号出现多行，实际箱货关系由版本化装载分配表达。**箱号与实际出运日期均迟绑定**；来源状态、缺时区时间和推导时间都不能直接成为实际业务事实。
...
> 适用边界修正（2026-09-23）：本文只继续约束**当前已实现的备货单兼容导入入口**。出运后目标模型的四张维护表字段覆盖下限、分阶段必填、只读 `container_operational_view` 和扩展治理，以
> [`POST_DEPARTURE_CORE_MODEL_GAP_V1` §5.3–§5.9](./POST_DEPARTURE_CORE_MODEL_GAP_V1.md) 为评审基线；负责人批准后由新的公共字段目录版本实例化，不在本文中静默改写既有 V1.2 运行语义。
...
| --------------- | -------- | ------ | ---- | ------------------ | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- | ---- |
| orderNumber     | 备货单号 | string | 是   | ReplenishmentOrder | `replenishment_order.order_number`；兼容快照写 `container_record.order_number` | 租户内备货单身份；旧导入按其分组，不能据此限制箱货基数                                 | O    |
...
│   └── ContainerCargoAllocationSet / Allocation（版本化实际装载）
...
产品品类字段不再默认走“每字段一列”。核心事实、合规专业档案、JSONB 扩展属性、元数据模板和搜索投影的分层及升格门禁，统一引用[产品属性与元数据治理](./PRODUCT_ATTRIBUTE_GOVERNANCE.md)。本文仍只负责导入目标字段的稳定语义和版本，不承担动态表单或产品主数据定义。
...
| orderNumber                                        | 备货单号（**备货单身份/旧导入分组锚**，先于箱号） | string   | 是（V1.0） | 备货单域                                                                                               | ✅       | 不作货柜稳定身份；采购阶段为采购订单号(PO)，属不同对象                                    |
...
## 3. 旧格式导入匹配键 / 重复判定（负责人已确认，2026-09-20 澄清边界）

- **分组锚 = `orderNumber`（备货单号）**：与库内备货单一致命中 → 更新；未命中 → 新建（见 [IMPORT_DOMAIN_MODEL](./IMPORT_DOMAIN_MODEL.md)）。当前兼容写入可同步更新货柜记录，但不得据此推定一单一柜。
...
- SKU 经 Master Data 公共 Port 核验绑定后，装载分配才能以 `containerId + replenishmentOrderLineId + quantity + unit` 形成权威关系；同一行可拆柜，一柜可含多单。
...
- 备货单号是备货单身份和旧导入分组锚，不是货柜稳定身份或一单一柜约束；箱号与实际出运日期迟绑定——§3。
