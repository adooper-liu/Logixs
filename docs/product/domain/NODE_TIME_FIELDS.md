# 节点时间字段清单 v0.2（NODE_TIME_FIELDS · 候选）

> 状态：候选 · 2026-09-16 对齐填空表 §2.1 与时间事实规则 · 用途：把 14 流程节点落成 schema 字段名草案，**尚未升格**。
...
| 7   | 清关       | `customs_plan`     | `customs_actual`          | ts   | 实际迟绑定 | 海关/报关     | 完成状态与可核验放行时间必须成对；缺时间不得生成实际事件                              |
...
| 12  | 卸柜       | `unload_plan`      | `unload_completed_actual` | ts   | —          | 仓库/WMS      | 实际值为现场/WMS 可核验的卸货完成时间；驱动 `unloaded`                                |
| 13  | 卸空       | `empty_plan`       | `empty_confirmed_actual`  | ts   | 迟绑定     | 仓库/WMS/手工 | 实际值为箱内卸净/空箱确认；推导值另存 `empty_estimated`，不生成实际事件               |
...
- 每个跨系统时间事实须满足 LIFECYCLE_CONSISTENCY R10：保留原始值、来源时区/偏移、UTC 标准值、来源系统与证据。
- 历史“卸空日期”、取空时间或按计划推导出的时间不得直接映射 `unload_completed_actual` / `empty_confirmed_actual`；只能作为原始声明或 `timeKind=estimated`、`captureSource=system_derived` 的估计进入对账。
- 入库归 WMS，不入此表。
