...
> （[P2_REVIEW_CHECKLIST](./P2_REVIEW_CHECKLIST.md) 及 D1–D10、`docs/product/domain/*`）逐维对照。
...
3. **来源（source）枚举已有实证**：外部数据路由已接受 `dataSource: Feituo/AIS/ShipCompany/Terminal/User/Excel`，`ContainerStatusEvent.dataSource`、`PortOperation.data_source` 已存来源。TO-BE 来源权威（D7）落地时需要**来源枚举集**——可直接用现网这套，清单未列。
