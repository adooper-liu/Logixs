...
**推荐批准：Shipment 表示一个租户内、已经实际离港、按主要承运运输单证和主路线稳定识别的运输执行。** 它覆盖从起运港离港到最终交付/关闭的同一执行，不是计划、Booking、HBL、货柜或 SKU 的别名。
...
- 没有稳定外部 Shipment ID 时必须进入人工匹配；`tenant_id + carrier_code + normalized_master_bill_number` 只能作为匹配候选和冲突检索键，不能在无人确认时自动生成永久 Shipment 身份。缺承运人或 MBL、多匹配、改单歧义一律阻断，不用船名、航次、ETD 或导入时间凑键。
- `shipment_number` 是租户内稳定的内部业务号，唯一但不是主键；不得直接复用上游计划号。
- 多 HBL、多个上游备货单和多个柜是 Shipment 的子关系；它们的更正使用追加版本，不复制 Shipment。

因此真实样本 `NBOZ9FF56400` 只能在承运人代码及单证权威确认后形成一个正式 Shipment；`SP20260830` 仍是上游计划引用，`A/B/C/D` 是其运输单证。
...
```text
T1 integration-import + shipment-registry（同库、一次批次事务）
  锁定批次/来源身份/柜次
...
- T1 失败回滚整批正式业务数据；不能保留半票关系。大文件先在 staging/precheck 处理，不以拆事务绕过原子要求。
...
6. 无稳定外部 Shipment ID 时必须人工匹配；承运人+MBL 只作候选检索，不自动建立永久身份。
...
“已订舱”“预计开船”“已装箱”均不满足准入。当前导入创建 `not_shipped` 记录的行为不得延续为目标入口。
...
| 稳定外部 Shipment ID   | 缺失则人工匹配                                      | 必填                  | 不能用拼接字段自动创建永久身份                           |
| `stuffingSnapshotRef`  | 可空                                                | 必填                  | 当前四表没有正式装箱快照；未来平台必须引用已接受版本     |
| SKU 装载分配           | 缺失时仅允许显式 `cargo_detail_incomplete` 兼容路径 | 必填                  | 整柜件重体不得反推 SKU 数量                              |
| departure proof        | 必填                                                | 必填                  | ATD、权威 `DEPARTED` 或授权人工确认三选一；loaded 不合格 |
| 清关/内陆/仓库切片     | 有值才提交                                          | 可选/迟绑定           | 空值表示尚未发生，不能生成未发生事件                     |
| 原始文件/来源行        | 必填                                                | 不适用                | 文件 profile 必须保留批次、行、映射版本和哈希            |

四表样本目前只有“出运日期”日期值、中文港口/船司值及 `已出运` 来源状态；在来源时区、代码映射和状态权威策略确认前，只能形成 `review_required`，不能伪造 `actualDepartedAt` 后直接 accepted。
...
T1 的原子范围是：交接记录、Shipment、出运货物、运输单证、柜次及关系、装载分配、上游引用、逐对象结果和待发布 Outbox。任何一步失败时，这一组核心事实同成同败，不保留孤立柜或半票 Shipment。
...
| `SHIPMENT_IDENTITY_REVIEW_REQUIRED`  | review_required | 无稳定外部 Shipment ID 或存在多匹配      |
| `DEPARTURE_EVIDENCE_INSUFFICIENT`    | review_required | 只有装船/预计/日期不完整，不能证明已离港 |
...
| `CARGO_DETAIL_INCOMPLETE`            | review_required | 兼容来源无 SKU 装载明细                  |
...
| ATD                 | `2026-09-18` 日期                                                | 不可写精确 `TIMESTAMPTZ` 或正式 departed 事件 | 无具体时刻和 UTC 偏移                                                       |
| 外部 `DEPARTED`     | 备货平台“已出运”                                                 | 当前不可单独触发正式落账                      | 仍需批准该来源的权威策略、状态映射版本和证据要求                            |
| 到港及后段事件      | 无                                                               | 不可生成                                      | ETA 不是 ATA；样本无实际到港、提柜、送仓、卸空或还箱事实                    |

这组样本可用于预检、草稿归组、单证关系和货物对账测试；在来源权威规则批准前，不能作为“正式 Shipment 已离港事件已成立”的成功 fixture。
...
| 5   | 整批正式导入事务                     | 不满足   | 当前逐备货单事务并可部分落账；目标需明确 all-or-nothing 或用户确认的分组提交，默认整批原子 |
...
2. **Migrate**：按证据回填。真实样本只能从稳定总提单/计划关系创建候选 Shipment；`2026-09-18` 只有日期且无 UTC 偏移，不得伪造 ATD 时间或正式事件。
