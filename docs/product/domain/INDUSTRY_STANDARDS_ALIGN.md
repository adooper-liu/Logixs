...
| # | 纠偏 | 落到 |
| --- | --- | --- |
| W1 | **外部事件统一信封**：`eventCode + eventTime + isEsti(预计/实际) + eventPlace/portCode/terminal + 标准描述 vs 原始值 + source`；我方状态/时间写入采纳 isEsti 双态与"标准/原始双轨"（对账多源） | INTEGRATION / LIFECYCLE R0 / 事件契约 |
