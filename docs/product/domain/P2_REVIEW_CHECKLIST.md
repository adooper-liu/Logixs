...
| IMPORT_DOMAIN_MODEL（P2-03）    | ☑ 通过 ☐ 需改 | 已改：一单一柜、主锚、已出运入口；准入证据、D7/D8 批次次序/升级（候选·样本回验后再校准）                  | 2026-09-12 |
| TARGET_FIELD_CATALOG            | ☑ 通过 ☐ 需改 | 已改：柜型标准码、orderNumber 主锚、迟绑定与已出运边界；必填硬闸/真实样本（候选·样本回验后再校准）        | 2026-09-12 |
