...
口语「过站」指完成当前节点；完成后通常激活下一节点。完整释义见 [GLOSSARY](../GLOSSARY.md)「过站」与填空表 [LIFECYCLE_NODE_IO_CATALOG](./LIFECYCLE_NODE_IO_CATALOG.md) §1.1。本契约不另造第三套状态。
...
- P6.1 签署只升格业务语义；后续 G6 已补齐状态、流程、节点、命令、阻断、转换、纠偏和查询 Schema，但仍未新增运行时代码、数据库迁移或 API。
...
生命周期错误码、HTTP 映射、冲突分类和错误信封统一引用[公共错误契约 V1](./PUBLIC_ERROR_CONTRACT_V1.md)。本文件只负责各错误的领域触发条件。重复同内容返回原结果，不作为错误。
...
P6.1 已由生命周期域负责人刘志高签署，Codex 完成技术边界复核；G6 随后完成 JSON Schema、覆盖索引及正负向 fixture 自校验，`GC-002` 已进入 `D4`。`completionEligibleNodeCodes` 的 V1 属性及节点映射已纳入整体批准；生成类型、实现、迁移和运行验证仍待 G7，当前唯一活动任务释放前不派生代码实施任务。
