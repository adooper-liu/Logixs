# 领域上下文与聚合边界（CONTEXT_MAP · v0.4 人话重构）
...
| Shipment | 货柜业务事实（一单一柜记录+状态机） | **ContainerRecord(主锚=备货单号)**；ShipmentPlan(候选) | P2-01~02/06 | O |
| Import | 把文件可靠导进来(批次/预检/审核/对账) | ImportBatch | P2-03/06 | O |
| AI Governance | 能力/风险分级/预算/审批令牌 | AiCapability | P2-11 | O |
| Audit | 写操作留痕 | 审计读模型 | P2-07 | O |
| 相邻 | integration/exception/notification/reporting/workflow | — | 各切片 | O |
...
| 边界 | Logix 本体=货柜收端+与 WMS 对接+控制塔；仓储内业为扩展线(VISION) |
...
- 待定：ShipmentPlan/Booking 归属、写端口命名、integration 边界细化。
