...
| is*rolled/operator/container_holder/tare/total/over*\*/danger_class | 柜况扩展                     | —              | FeiTuo        | 映射；超限/危险品入标记候选（MARKER） |
...
| status*code/status_occurred_at/has_occurred/location*\*/lat/lng/timezone/data_source | 外部节点原样               | —            | API      | 保留为事件/原始值（W1 信封）          |
...
| 现网列                                                            | 落点                               | 处置                                          |
| ----------------------------------------------------------------- | ---------------------------------- | --------------------------------------------- |
| warehouse\_\* / planned_actual_warehouse / arrival                | 送仓→卸柜前                        | 映射（入库仓库，K11）                         |
...
| `ext_demurrage_standards`/`ext_container_charges`                                             | 费用标准/记录        | 映射（P4 超期费用；三类边界与多阶段金额见 FEE_DEMURRAGE） |
