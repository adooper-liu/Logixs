...
| ------------------- | -------------- | ----------------------- | --------------------------------------------------------- | ------------------------------ | ---------------------------------- |
| `confirm_stuffing`  | 确认装箱定稿   | 节点#2 装箱（已装未出） | 真实毛重/件数/体积/封号、箱号（迟绑定回填）/ 内部状态推进 | 操作员 / 中（需复核确认）      | 走合法转换；需 R4 可写窗口与证据齐 |
| `confirm_departure` | 确认出运/离港  | 节点#3/#4 出运·离港     | atd/出运时间 / 内部                                       | 操作员 / 中（需复核确认）      | 前进式写入                         |
| `confirm_arrival`   | 确认到港       | 节点#8 目的港           | ata / 内部                                                | 操作员 / 中（需复核确认）      |                                    |
| `confirm_unstuffed` | 确认卸空可还箱 | 节点#13 卸空            | 卸空时间 / 内部                                           | 操作员+仓库 / 中（需复核确认） | 触发后续还箱待办                   |
...
- **状态推进类（confirm\_\*）默认采用中等级复核确认；只有契约明确提供可撤销窗口时才可降级（负责人采纳 2026-09-04，评审 A3）**：一旦推进即进入密封区（R3）难回改，禁止"低确认一键即推"。
