...
- 旧候选 `VAL_*`、`AUTH_*`、`BIZ_*`、`IDEM_*`、`NF_*`、`EXT_*` 只作为迁移输入，不得在新 V1 API 发布；迁移映射由 P6 Schema fixture 固化。
...
当前 `PublicErrorCodeV1` 和 `ErrorResponseV1` 已在 P6 达到 `D4`；尚无生成类型、OpenAPI、数据库映射或运行时异常映射器。P7 实现各技术载体及 parity 检查。
