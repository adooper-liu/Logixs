...
G6 已补齐公共命令、动作定义、授权上下文、授权决定和复核记录 Schema，并通过覆盖索引及正负向 fixture 自校验，门禁为 `D4`。G7 已登记首个具备现有受保护写链路的真实动作 `record_lifecycle_date_fact`，其动作定义由公共 Schema 校验，柜综合投影按能力、Flow 状态和并发版本返回允许动作；其余动作目录、通用命令接收器与完整服务端策略对拍仍未完成，不据此提升全局门禁。
