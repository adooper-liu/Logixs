...
tenantScope?
factType or eventCode or fieldCode
subjectType
jurisdiction?
direction?
locationRole?
...
该矩阵只给出原则；任务阶段 G6 实例化必须使用复合策略，不得编码成全局来源排序。
...
- G6 已补齐公共证据记录、来源策略、验证裁决、证据关系和人工锁 Schema，并通过覆盖索引及正负向 fixture 自校验，门禁为 `D4`；尚无生成类型、数据库迁移或运行时裁决器。
- 任务阶段 G7 将派生技术载体，并继续实例化来源策略映射和测试。
