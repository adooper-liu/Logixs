...
> 状态：**已过时（历史清洗假设）** · 2026-09-20 · 负责人：刘志高。
> 替代：箱货关系以 [SHIPMENT_FLOW_OVERVIEW](./SHIPMENT_FLOW_OVERVIEW.md) 的版本化 `ContainerCargoAllocation` 为准；本文保留旧的一单一柜调查方法，不得继续作为目标模型或预检规则。
...
- 不做：根据脏数据自动合并/拆分备货单或货柜；产品明细模型按已确认的一单多产品关系另行实施。
...
4. 同备货单号多产品行是合法关系，不作为异常；只有同备货单指向多个不同货柜、聚合表头冲突或同箱跨多备货单时进入人工处置。
