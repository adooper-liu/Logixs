...
DUA admitted
  ...
  -> Levante recorded
  -> release candidate（context.jurisdiction/direction 区分适用范围）
...
本样本首次提供了可区分 acceptedAt 与 releasedAt 的跨场景放行候选，也证明风险通道不能等同于流程终态。经西班牙业务负责人确认后，可把 `DUA + Levante` 升格为注册事件 `release` 的辖区特定映射来源；辖区与方向进入 context，不另造事件码。
