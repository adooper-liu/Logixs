...
> 状态：**正式 V1（负责人批准）** · 契约 ID：`GC-003` · 版本：`1.1.0` · 2026-09-18 · 负责人：刘志高。
...
14 节点的完成资格事件统一允许 `planned | estimated | actual`：计划与预计只形成日期投影，预计值只在有可靠业务来源时记录；只有核验合格的 `actual` 才能申请完成节点。这个规则扩展日期能力，不改变事件码、完成资格或状态机守卫。
