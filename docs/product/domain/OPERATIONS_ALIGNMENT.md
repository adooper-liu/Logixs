...
| OA-04  | 真实箱号建档后按管道展开全部 NodeTask                | 任务进入全局池；节点位置与事实重算就绪条件 |
| OA-05  | NodeTask 预生成一张或多张 WorkOrder                  | 按责任、事务等待或独立验收结果拆单         |
| OA-06  | 动作必须关联具体工单并形成操作、数据、证据与同步回执 | 动作是写入入口，不是流程状态               |
| OA-07  | 状态按“工单 → 工序任务 → 合法事件 → 主流程”推进      | 禁止越级改流程状态                         |
| OA-08  | 人工或外部事实可先于节点进入                         | 先重算任务条件；核验实际事件仍按顺序推进   |
...
| taskCondition   | NodeTask        | 事实计算适用性、就绪度和完成资格     |
