...
| [ADR-012](./ADR-012-external-timestamp-timezone.md) | 外部来源时间一律带「时区来源」；供应商声明 / 地点配置 / 人工确认三档均可驱动不可逆动作，未解析进复核。 | accepted |
...
- 2026-09-18：ADR-012 由云当网/飞驼适配调研触发；草拟后就两项开放问题完成负责人评审——① 时区来源采用「供应商声明 / 地点配置 / 人工确认」三档，**地点配置解析出的确定偏移可驱动不可逆动作**（原稿一律降级为复核会使供应商集成退化为只读）；② 覆盖范围统一到导入与供应商一套形态。修订后经负责人**接受**（accepted）。
