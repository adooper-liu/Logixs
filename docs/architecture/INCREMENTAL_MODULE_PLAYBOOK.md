...
| 0.2 | Activity / 下次动作      | 「到期做什么、谁负责」投影到工单/任务，不另造待办引擎 | `work-execution` / `lifecycle-control` | 通用个人 TODO 产品             | 待开                 |
...
在 P0 未收口前，默认按下序选一刀（仍须单独 brief）：

1. **0.1+0.2**：对象活动流 + Activity 投影到任务（通知挂 `containerId`/`taskId`）。
2. **0.3 深化**：助手回复附允许动作说明与只读对象摘要字段。
3. **1.1**：第一条可配置自动化（样板可继续用 outbox 死信 → 通知，抽出规则表）。
