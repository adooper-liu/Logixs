...
verification: https://github.com/adooper-liu/Logixs/pull/45
---

# 任务：已出运 Shipment 生命周期技术基础

> 唯一设计依据：[出运后核心模型差异 V1](../../product/domain/POST_DEPARTURE_CORE_MODEL_GAP_V1.md)。
> 本任务实现负责人已批准的 Shipment 粒度、统一 Handoff、加法迁移、查询投影与真实样本保护边界，不在 brief 复制第二套字段定义。
> 2026-09-23 方向校准：本任务在 PR #45 交付的技术基础处结束，不再以一个大任务承诺全部岗位闭环；用户界面按
> [出运后工作台交付基线](../../product/POST_DEPARTURE_WORKBENCH_DELIVERY_BASELINE.md) 串行拆分。

## 目标

建立已出运 Shipment 的数据库、公共契约、预检/接收、查询投影和下游初始化基础，供后续岗位界面按小切片消费。
...
- [x] 真实不完整样本进入 `review_required`，不伪造 ATD；正式接管界面和人工补全移入 `UI-01`。
- [x] `container_operational_view` 首个只读聚合、租户隔离和真实 PostgreSQL 集成测试完成；十张详情的运行时逐值 UI 对拍移入 `UI-02`。
- [x] 服务端认证授权、分页、稳定错误码、并发和租户隔离测试通过。
- [x] PR #45 `quality`、契约门禁和专项迁移验证通过；本地 `pnpm validate` 仅被未跟踪资料的 8 张缺图阻断。
...
- PR #45 必需 CI 全部通过并合入 `main`。本任务不代表用户可操作的 Shipment 生命周期已经闭环；剩余工作以 `UI-01`～`UI-07` 分段交付。
...
| 2026-09-23 | done   | Codex | `a0ccadb` | PR #45 合入；技术基础任务收口。岗位 UI 与真实业务闭环按工作台基线拆成串行小切片，下一任务为 `p6-post-departure-handoff-workbench-first-slice.md`。                                                                                                                                                                                                 |
