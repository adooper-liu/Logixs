...
> 🗣️ 人话对照：[代码怎么往前推](./PROJECT_BOOTSTRAP_PLAIN_LANGUAGE.md)（只说明，不改本清单勾选与门禁）。
...
- [x] P5-01 完成威胁建模：租户隔离、文件上传、提示注入和 Tool 越权。
...
**P0/P1 已完成；P2-05 最小访问模型与 P5-01 威胁模型已定，工程基线、导入闭环、作业壳、内陆计划草稿与费用计算第一刀均已落地。** 下一条主线是按 [ADR-008](../architecture/decisions/ADR-008-oidc-oauth.md) 实现 **P5-02 Keycloak OIDC、默认拒绝的能力授权与对象范围校验**。正式身份和服务端授权建立后，再实现内陆计划确认、资源占用与取消释放；不得在客户端可伪造的开发身份头上增加高风险写操作。

并行跟踪（不阻塞实现开工评审）：

1. 真实现状导入流程与错误点现场观察（[RAID](./RAID.md) I-02，负责人：刘志高）。
2. 首批真实用户、IdP 组映射、组织/地点主数据与计划确认人（P5-02）。
3. 黄金样本与主数据采集（P2-12 / P7）。
4. 历史 CI 失败的根因之一是 Prisma Client 双副本；`database/schema.prisma` 已改为单一输出 `generated/prisma`。此后合入仍须 `quality` 通过。
