...
> 状态：**历史摘要（箱货基数已被后续决策取代）** · 2026-09-20 · 原任务载体：`p2-shipment-import-domain.md`。
> 当前实施口径见 [业务纵向交付路线图](./DOMAIN_VERTICAL_DELIVERY_PLAN.md) 与 [SHIPMENT_FLOW_OVERVIEW](../product/domain/SHIPMENT_FLOW_OVERVIEW.md)。
...
- 当前决策：备货单产品行与货柜通过版本化装载分配形成 N:M；支持一柜多备货单和产品行拆柜。
- 备货单号是备货单身份/旧导入分组锚，货柜稳定主锚是 `containerId`；箱号/实际出运日期仍为迟绑定。
